    ...
    while (true) {
      ...
      if (!advance) {
        ...
        if (REQUEST == request_t::PDP && (node_stack.top().from_idx == node_stack.route_length ||
          ...
          time_between = node_stack.template get_dim_between<dim_t::TIME>(node_stack.top().from_idx,
                                                                 node_stack.top().intra_idx + 1);
...

{
  ...
  if (threadIdx.x == 0) {
    ...
    // insert request
    s_route.template insert_request<REQUEST>(request_locations, request_node, solution.route_node_map, true);
